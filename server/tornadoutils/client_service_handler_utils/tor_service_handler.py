...
TOR_SOCK = "/run/tor_transparent.sock"


async def call_tor_service(action: str, payload: dict = None) -> dict:
    """
    Sends a request to the Tor transparent proxy daemon socket.

    Usage:
        result = await call_tor_service("health")
        result = await call_tor_service("add_relay")
        result = await call_tor_service("stop_relay", {"id": "relay-1"})
        result = await call_tor_service("relay_circuits", {"id": "relay-0"})
        result = await call_tor_service("logs", {"id": "relay-0", "lines": 50})
        result = await call_tor_service("routing_status")
        result = await call_tor_service("apply_routing")
        result = await call_tor_service("remove_routing")
    """
    try:
